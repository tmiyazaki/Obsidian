...
- **Agentic AI**: 自律的に多段階タスクを実行する AI 設計。 → [[30-Interface/Agentic-AI-Patterns]]
- **AI Augmented Development**: Cursor/Copilot/Claude Code などで開発を加速。 → [[40-Bridge/AI-Augmented-Development]]
- **AI Eval / Red Teaming**: AI の品質と安全性の体系的評価。 → [[40-Bridge/AI-Evaluation-Safety]]
...
- **Barge-in**: AI 応答中にユーザーが割り込んで止められること。 → [[30-Interface/Realtime-Multimodal-AI]]
- **Branded Type**: TypeScript で型エイリアスを強化し、誤代入を型レベルで防ぐ。 → [[10-Coding/Type-Systems-and-DDD]] / [[10-Coding/Multi-tenancy]]
...
- **Cascaded vs Native Multimodal**: 連結式 (STT→LLM→TTS) と一体型モデル。 → [[30-Interface/Realtime-Multimodal-AI]]
- **Citation (出典)**: AI 応答に引用元を併記して幻覚を抑える。 → [[30-Interface/AI-LLM-Interfaces]] / [[10-Coding/Embeddings-RAG]]
...
- **Computer Use**: AI が画面を見て操作する自動化(Claude/Browser Use)。 → [[30-Interface/Agentic-AI-Patterns]]
- **Conversational UI**: 「選ぶ」から「話す」へ移行する設計。 → [[30-Interface/Conversational-UI]]
- **Container Query**: 親要素サイズで切り替わる CSS 機能。 → [[30-Interface/Responsive-Design]] / [[10-Coding/Modern-Web-Platform]]
- **CRDT (Conflict-free Replicated Data Type)**: 任意順で同じ結果になるデータ構造。協働編集の基盤。 → [[30-Interface/Real-time-Collaboration]] / [[40-Bridge/Local-First-Sync]]
...
- **Effect-TS**: 副作用とエラーを型で扱う関数型 TS フレームワーク。 → [[10-Coding/Schema-Driven-Stack]]
- **Embedding (埋め込み)**: 文・画像をベクトルに変換。RAG の基礎。 → [[10-Coding/Embeddings-RAG]]
...
- **Generative UI**: LLM が UI コンポーネントを動的に組む。 → [[30-Interface/Generative-Streaming-UI]]
- **Golden Dataset**: AI 評価で「ここで失敗してはいけない」テストケース集。 → [[40-Bridge/AI-Evaluation-Safety]]
- **Guardrails**: AI の入出力を実行時に検査する安全装置。 → [[40-Bridge/AI-Evaluation-Safety]]
...
- **Hallucination (幻覚)**: AI がもっともらしい嘘を生成する現象。 → [[40-Bridge/AI-Evaluation-Safety]]
- **Hick's Law**: 選択肢が増えるほど決定が遅くなる。 → [[30-Interface/Mental-Models]]
- **HITL (Human-in-the-Loop)**: 重要操作で人間の承認を介在させる設計。 → [[30-Interface/Agentic-AI-Patterns]]
...
- **Local-first**: オフラインでも動き、復活時に同期するソフトウェア哲学。 → [[40-Bridge/Local-First-Sync]] / [[30-Interface/Real-time-Collaboration]]
...
- **MCP (Model Context Protocol)**: LLM とツール/データを繋ぐオープンプロトコル (Anthropic)。 → [[30-Interface/Agentic-AI-Patterns]]
- **MediaPipe / TensorFlow.js**: ブラウザでの ML (顔・手・姿勢検出)。 → [[30-Interface/Sensor-Camera-Haptics]]
- **Multi-tenancy**: 複数テナントで同じインフラを共有する SaaS 設計。 → [[10-Coding/Multi-tenancy]]
- **Multimodal**: テキスト・音声・画像・映像を同時に扱う AI。 → [[30-Interface/Realtime-Multimodal-AI]]
...
- **pgvector**: Postgres でベクトル検索を可能にする拡張。 → [[10-Coding/Embeddings-RAG]]
- **Pricing UX**: 価格・課金体験の設計(信頼の前線)。 → [[30-Interface/Pricing-Monetization-UX]]
- **Privacy by Design**: プライバシーを設計の前提にするアプローチ (Cavoukian)。 → [[40-Bridge/Privacy-by-Design]]
- **Prompt Injection**: ユーザー入力でシステムプロンプトを乗っ取る攻撃。 → [[40-Bridge/AI-Evaluation-Safety]]
...
- **RAG (Retrieval-Augmented Generation)**: 検索 + 生成で LLM に外部知識を持たせる。 → [[10-Coding/Embeddings-RAG]]
- **ReAct**: Reasoning + Acting を交互に行うエージェントパターン。 → [[30-Interface/Agentic-AI-Patterns]]
- **Race Condition**: 並行実行で順序依存の不整合が起きるバグ。 → [[10-Coding/Concurrency-Async]]
- **Replicache / ElectricSQL / Zero**: ローカルファースト同期エンジン。 → [[40-Bridge/Local-First-Sync]]
- **Reranking**: 検索結果を 2 段階で精度上げる手法。 → [[10-Coding/Embeddings-RAG]]
- **RSC (React Server Components)**: サーバでレンダされる React コンポーネント。 → [[30-Interface/Generative-Streaming-UI]]
...
- **Schema-Driven**: 1 スキーマから型・検証・ドキュメント・SDK を生成。 → [[10-Coding/Schema-Driven-Stack]]
- **Server Actions**: React 19/Next.js のサーバ関数を直接呼ぶ機能。 → [[10-Coding/Modern-Web-Platform]] / [[30-Interface/Generative-Streaming-UI]]
...
- **TTFT (Time To First Token)**: AI 応答の最初のトークンが届くまでの時間。 → [[30-Interface/Realtime-Multimodal-AI]] / [[30-Interface/Generative-Streaming-UI]]
...
- **View Transitions API (cross-document)**: MPA でも遷移アニメ可能な新標準。 → [[10-Coding/Modern-Web-Platform]] / [[20-Design/Motion-System]]
...
- **WebGPU**: ブラウザで GPU 計算 (3D + ML)。WebGL の後継。 → [[10-Coding/Modern-Web-Platform]] / [[20-Design/Creative-Coding-Canvas-WebGL]]
- **WebRTC**: P2P の超低レイテンシ通信。ビデオ通話・コラボ。 → [[30-Interface/Real-time-Collaboration]] / [[30-Interface/Realtime-Multimodal-AI]]
